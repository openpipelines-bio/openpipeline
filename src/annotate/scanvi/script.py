...
def main():
    ...
    input_modality.obsm[par["output_obsm_scanvi_embedding"]] = (
        scanvi_query.get_latent_representation()
    )

    logger.info("Running predictions on query data")
    input_modality.obs[par["output_obs_predictions"]] = scanvi_query.predict(
        input_modality
    )
    input_modality.obs[par["output_obs_probability"]] = np.max(
