...
def test_gene_type_column(run_component, subset_input_gtf, random_path, attributes):
    ...
    with gzip.open(output_gtf, "rt") as f:
        unique_gene_types = {match for line in f for match in re.findall(r'gene_type "([^"]*)"', line.split('\t')[8])}
    assert set(attributes) == unique_gene_types, "Output GTF does not contain exactly the expected gene types."
...
def test_different_columns(run_component, subset_input_gtf, random_path):
    ...
    args = [
        "--input_gtf", subset_input_gtf,
        "--output_gtf", output_gtf,
        "--attribute", "gene_type:transcribed_unprocessed_pseudogene;transcript_id:ENST00000456328.2"
    ]
    ...
    with gzip.open(output_gtf, "rt") as f:
        wrong_attributes_count = sum(
            1 for line in f
            if dict(re.findall(r'(\S+) "([^"]*)"', line.split('\t')[8])).get("gene_type") != "transcribed_unprocessed_pseudogene" and
                dict(re.findall(r'(\S+) "([^"]*)"', line.split('\t')[8])).get("transcript_id") != "ENST00000456328.2"
        )
    assert wrong_attributes_count == 0, "Output GTF contains unexpected attribute values."
