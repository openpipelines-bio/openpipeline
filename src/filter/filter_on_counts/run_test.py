...
assert list(mu_in.mod["rna"].var["feature_types"].cat.categories) == [
    "Gene Expression"
], "Feature types of RNA modality should be Gene Expression"
assert list(mu_in.mod["prot"].var["feature_types"].cat.categories) == [
    "Antibody Capture"
...
assert (
    mu_out.mod["prot"].n_obs <= orig_prot_obs
), "Some prot obs should have been filtered"
assert (
    mu_out.mod["prot"].n_vars <= orig_prot_vars
), "Some prot vars should have been filtered"
