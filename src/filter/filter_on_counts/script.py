...
meta = {
    'functionality_name': 'filter_on_umi_genes_and_mito'
}
### VIASH END
...
for mod in par['modality']:
    ...
    if par["min_cells_per_gene"] is not None:
        print(f"  Removing genes with non-zero values in <{par['min_cells_per_gene']} cells.")
    ...
    data.obs['pct_mito'] = np.ravel(np.sum(data[:, mito_genes].X, axis=1) / np.sum(data.X, axis=1))
    ...
    if par["min_fraction_mito"] is not None:
        print(f"  Removing cells with <{par['min_fraction_mito']} percentage mitochondrial reads.")
        n_obs_before = data.n_obs
    ...
    if par["max_fraction_mito"] is not None:
        print(f"  Removing cells with >{par['max_fraction_mito']} percentage mitochondrial reads.")
        n_obs_before = data.n_obs
