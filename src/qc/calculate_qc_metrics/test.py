...
# def test_log1p_flag_adds_derived_columns(
#     run_component, mudata_with_boolean_column, random_path, file_format
# ):
#     output_path = random_path("h5mu" if not file_format == "zarr" else "zarr")
#     if file_format == "zarr":
...
#     else:
#         data_with_qc = md.read(output_path)
#     rna_var = data_with_qc.mod["rna"].var
#     rna_obs = data_with_qc.mod["rna"].obs

#     var_pairs = [("obs_mean", "log1p_obs_mean"), ("total_counts", "log1p_total_counts")]
#     for raw, log1p_col in var_pairs:
#         np.testing.assert_allclose(
#             rna_var[log1p_col].to_numpy(),
#             np.log1p(rna_var[raw].to_numpy()),
#         )
#     obs_pairs = [
#         ("total_counts", "log1p_total_counts"),
#         ("num_nonzero_vars", "log1p_num_nonzero_vars"),
#         ("total_counts_custom", "log1p_total_counts_custom"),
#     ]
#     for raw, log1p_col in obs_pairs:
#         np.testing.assert_allclose(
#             rna_obs[log1p_col].to_numpy(),
#             np.log1p(rna_obs[raw].to_numpy()),
#         )


# def test_compare_scanpy_log1p(
#     run_component, mudata_with_boolean_column, input_mudata, random_path, file_format
# ):
#     output_path = random_path("h5mu" if not file_format == "zarr" else "zarr")
#     run_component(
...
#         "total_counts": "total_counts",
#         "log1p_mean_counts": "log1p_mean_counts",
#         "log1p_total_counts": "log1p_total_counts",
#     }
...
#                 check_dtype=False,
#             ),
#             f"from_var: {from_var}, to_var: {to_var} VAR KEYS SC {scanpy_var.keys} VAR COMP KEYS {component_var.keys()}",
#         )
