...
def test_cross_check(run_component, random_path):
    ...
    assert sum(output_mudata.mod["rna"].var["id_in_vocab"]) != len(output_mudata.mod["rna"].var["id_in_vocab"]), "Genes were not filtered."

    output_hvg_path = random_path(extension="h5mu")
    args_hvg = [
        "--input", input_path,
        "--output",  output_hvg_path,
        "--modality", "rna",
    ...
    assert sum(output_mudata_hvg.mod["rna"].var["id_in_vocab"]) != len(output_mudata_hvg.mod["rna"].var["id_in_vocab"]), "Genes were not filtered."
    assert sum(output_mudata.mod["rna"].var["id_in_vocab"]) != len(output_mudata_hvg.mod["rna"].var["id_in_vocab"]), "Genes were not filtered based on HVG."
