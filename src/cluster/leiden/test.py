...
def mudata_custom_connectivities_key(input_data, random_h5mu_path):
    ...
    result.mod['rna'].obsp["custom_connectivities"] = result.mod['rna'].obsp['connectivities'].copy()
...
def test_leiden(input_path, run_component, random_h5mu_path, compression, output_key, expected_output_key):
    output_path = random_h5mu_path()
    args = [
        "--input", input_path,
        "--resolution", "1:0.25",
    ...
    assert expected_output_key in data.mod["rna"].obsm, f"Expected to find key '{expected_output_key}' in .obsm"
    # check whether leiden.custom.resolution was found
    assert "1.0" in data.mod["rna"].obsm[expected_output_key].columns, 'Output should contain resolution 1.0.'
    assert "0.25" in data.mod["rna"].obsm[expected_output_key].columns, 'Output should contain resolution 0.25.'

def test_leiden_custom_connectivities_key(mudata_custom_connectivities_key, run_component, random_h5mu_path):
    output_path = random_h5mu_path() 
    run_component([
