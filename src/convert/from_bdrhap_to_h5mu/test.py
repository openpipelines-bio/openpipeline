...
assert np.array_equal(prot_adata.obs["library_id"].unique(), ["12ABC & 12SMK & 12WTA"]), "Antibody Capture .obs library_id should equal '12ABC & 12WTA."
assert "sample_tag" in prot_adata.obs.keys(), "Protein modality should contain column 'sample_id'."
assert "sample_id" in prot_adata.obs.keys(), "Protein modality should contain column 'sample_name'."
