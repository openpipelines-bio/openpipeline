...
"$meta_executable" \
  --input "$meta_resources_dir/WTA.bd_rhapsody.output_raw/sample_final.BAM" \
  --output "foo.bam" \
  -t 8 \
  --bam
...
readarray -t input_tags < <( samtools view --no-header "$meta_resources_dir/WTA.bd_rhapsody.output_raw/sample_final.BAM" | grep -oP "(?<=MA:Z:).*[\s]" )
