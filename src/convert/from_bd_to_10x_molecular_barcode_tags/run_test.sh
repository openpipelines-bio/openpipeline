...
./"$meta_functionality_name" \
  --input "$meta_resources_dir/WTA.bd_rhapsody.output/WTA_final.BAM" \
  --output "foo.bam" \
  -t 8 \
  --bam
...
readarray -t input_tags < <( samtools view --no-header "$meta_resources_dir/WTA.bd_rhapsody.output/WTA_final.BAM" | grep -oP "(?<=MA:Z:).*[\s]" )
