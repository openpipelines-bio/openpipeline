...
def proces_perturbation(key_name: str, mudatas: dict[str, mudata.MuData], efficiency_files: dict[str, Path]):
...
def process_counts(counts_folder: Path, multiplexing_info, metrics_files):
    ...
    if multiplexing_info:
        ...
        metrics_file = pd.read_csv(list(metrics_files.values())[0], 
        ...
        barcode_sample_mapping = sample_ids.loc[:,['Group Name', 'Metric Value']].set_index('Group Name').squeeze().to_dict() 
        return split_samples(mudata_all_samples, multiplexing_info, barcode_sample_mapping)
...
def process_metrics_summary(mudatas: dict[str, mudata.MuData], metrics_files: dict[str, Path]):
...
def process_vdj(mudatas: dict[str, mudata.MuData], vdj_folder_path: str):
    ...
    for _, mudata_obj in mudatas.items():
        json_for_sample = [entry for entry in json_obj if entry['barcode'] in mudata_obj.obs_names]
...
def get_modalities(input_data):
    ...
    for modality_name, modality_data_path in input_data.items():
        if modality_name in ("count", "multiplexing_analysis") or not modality_data_path:
            continue
        try:
