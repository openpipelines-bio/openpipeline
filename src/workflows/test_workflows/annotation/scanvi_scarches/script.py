...
def test_run():
    ...
    assert model_args["labels_key"] == par["obs_target"], f"Expected labels_key to be {par['obs_target']}, found: {model_args['labels_key']}."
    assert model_args["batch_key"] == par["obs_batch_label"], f"Expected batch_key to be {par['obs_batch_label']}, found: {model_args['batch_key']}."
