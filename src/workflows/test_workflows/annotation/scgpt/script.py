...
def test_run():
    ...
    expected_obsm = ["gene_id_tokens", "values_tokenized", "padding_mask", "bin_edges", "binned_counts"]
    ...
    assert input_mudata.mod["rna"].obsm["binned_counts"].shape[1] <= par["n_hvg"] + 0.1 * par["n_hvg"], f"Input shape should be lower or equal than --n_hvg {par['n_hvg']}, found: {input_mudata.shape[1]}."
