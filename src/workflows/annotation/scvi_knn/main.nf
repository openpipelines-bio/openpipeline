workflow run_wf {
  ...
  main:
    ...
    input_modalities_ch = alignment_ch
      // keep all keys apart from reference
      | map {id, state -> 
          def newState = state.findAll{it.key != "reference"}
          [id, newState]
        }
      // split modalities from query file
      | split_modalities.run(
        key: "split_modalities_input",
        fromState: {id, state ->
      ...
      | view {"After splitting input modalities: $it"}

    reference_modalities_ch = alignment_ch
      // | setState(["reference"])
      // split modalities from reference file
      | split_modalities.run(
        key: "split_modalities_reference",
        fromState: {id, state ->
          def newState = ["input": state.reference, "id": id]
        },
        toState: ["output": "output", "output_types": "output_types"]
      )
      | flatMap {id, state ->
        ...
        types.collect{ dat ->
          ...
          def new_id = id // it's okay because the channel will get split up anyways
          def new_data = outputDir.resolve(dat.filename)
          [ new_id, state + ["reference": new_data, modality: dat.name]]
        }
      }
      // Remove arguments from split modalities from state
      | map {id, state -> 
      ...
      | view {"After splitting reference modalities: $it"}

    output_ch = input_modalities_ch.mix(reference_modalities_ch)
