...
nondev_ix <- unlist(map2(
  c(1, dev_end + 1),
  c(dev_begin - 1, length(param_txt)), function(i, j) {
    if (i >= 1 && i < j) {
      seq(i, j, 1)
    } else {
      NULL
    ...
    arguments <- map2_dfr(
      ...
      function(arg_start, arg_end) {
        ...
        if (!grepl(second_regex, second_txt)) {
          stop(
            "Line '", second_txt,
            "' did not match regex '", second_regex, "'"
          )
        }
...
file_args <- c(
  "genomeDir", "readFilesIn", "sjdbGTFfile", "genomeFastaFiles",
  "genomeChainFiles", "readFilesManifest"
)
long_args <- c(
...
manual_args <- c(
  "runThreadN", "outTmpDir", "runMode",
  "outFileNamePrefix", "genomeDir", "readFilesIn"
)
...
yaml::write_yaml(
  ...
  "src/mapping/star_align/argument_groups.yaml"
)
