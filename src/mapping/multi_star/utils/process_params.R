...
nondev_ix <- unlist(map2(
  c(1, dev_end + 1),
  c(dev_begin - 1, length(param_txt)),
  function(i, j) {
    if (i >= 1 && i < j) {
      seq(i, j, 1)
    } else {
    ...
    arguments <- map2_dfr(
      ...
      function(arg_start, arg_end) {
        ...
        if (!grepl(second_regex, second_txt)) {
          stop(
            "Line '", second_txt, "' did not match regex '",
            second_regex, "'"
          )
        }
        type <- gsub(second_regex, "\\1", second_txt)
        desc_start <- str_trim(gsub(
          second_regex, "\\2",
          second_txt
        ))
        ...
        desc <-
          if (sum(str_length(desc_cont1)) == 0) {
            ...
            desc_cont3 <- ifelse(grepl("\\.\\.\\.", desc_cont2),
              paste0("- ", desc_cont2), desc_cont2
            )
...
# assigning types
type_map <- c(
  "string" = "string", "int" = "integer", "real" = "double",
  "double" = "double", "int, string" = "string"
)
file_args <- c(
  "genomeDir", "readFilesIn", "sjdbGTFfile",
  "genomeFastaFiles", "genomeChainFiles", "readFilesManifest"
)
long_args <- c(
  "limitGenomeGenerateRAM", "limitIObufferSize",
  "limitOutSAMoneReadBytes", "limitBAMsortRAM"
)
required_args <- c("genomeDir", "readFilesIn")

# converting examples
as_safe_int <- function(x) {
  tryCatch(
    {
      as.integer(x)
    },
...
manual_args <- c(
  "runThreadN", "outTmpDir", "runMode", "outFileNamePrefix", "genomeDir",
  "readFilesIn", "genomeTransformOutput", "genomeFileSizes",
  "genomeChrSetMitochondrial",

  # only for star_and_htseq
  "outSAMtype", "genomeLoad"
)
...
out2 <- out %>%
  ...
  filter(
    !grepl("--runMode", description) |
      grepl("--runMode alignReads", description)
  ) %>%
  filter(
    !grepl("--runMode", group_name) |
      grepl("--runMode alignReads", group_name)
  ) %>%
  mutate(
    viash_arg = paste0("--", name),
    type_step1 = type %>%
      str_replace_all(
        ".*(int, string|string|int|real|double)\\(?(s?).*",
        "\\1\\2"
      ),
    viash_type = type_map[gsub(
      "(int, string|string|int|real|double).*", "\\1",
      type_step1
    )],
    multiple = type_step1 == "int, string" |
      grepl("s$", type_step1) | grepl("^[4N][\\* ]", type),
    ...
    # turn longs into character because
    # yaml::write_yaml doesn't handle longs well
    viash_default = ifelse(sapply(viash_default, bit64::is.integer64),
      map(viash_default, as.character),
      viash_default
    ),
...
yaml::write_yaml(
  ...
  "src/mapping/star_and_htseq/arguments_star.yaml"
)
