...
def test_cellxgene_extract_metadata_expression(run_component, tmp_path):
    ...
    expected_obs = [
        "dataset_id", "assay", "assay_ontology_term_id", "cell_type",
        ...
        "tissue_general_ontology_term_id", "soma_joinid", "collection_id",
        "collection_name", "collection_doi", "dataset_title"
    ]
    ...
    expected_var = [
        "feature_id", "feature_name", "soma_joinid"
    ]
    ...
    for exp_layer in expected_layers:
        assert exp_layer in layers.keys(), f"Expected layer '{exp_layer}' not found in .layers"
