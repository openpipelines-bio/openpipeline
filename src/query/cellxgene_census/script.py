...
def write_mudata(query_data, path, compression, modality):
    logger.info("Writing MuData object to '%s'", path)

    mdata = mu.MuData({modality: ad.AnnData()})

    mdata.write_h5mu(path, compression=compression)

    mu.write_h5ad(path, data=query_data, mod=modality)
...
def main():
    ...
    write_mudata(query_data, par["output"], par["output_compression"], par["output_modality"])
