...
logger.info(f"Input (pseudo-)bulk data shape: {mod.shape}")
...
counts = pd.DataFrame(mod.X, columns=mod.var_names, index=mod.obs_names)
...
if par["filter_gene_patterns"]:
    ...
    logger.info(
        f"Filtered out genes matching patterns {par['filter_gene_patterns']}: {before_filter} -> {counts_filtered.shape[1]}"
    )
...
try:
    ...
    if par["filter_genes_min_samples"] is not None:
        logger.info(
            f"Filtering genes expressed in at least {par['filter_genes_min_samples']} samples"
        )
        ...
        logger.info(
            "Default filtering: removing genes with zero counts across all samples"
        )
    ...
    if par["per_cell_type"]:
        ...
        design_no_celltype = (
            par["design_formula"]
            .replace(f"+ {par['cell_type_column']}", "")
            .replace(f"{par['cell_type_column']} +", "")
        )
        ...
        for cell_type in metadata[par["cell_type_column"]].unique():
            ...
            logger.info(
                f"DESeq2 analysis for cell type {cell_type} completed. Found {len(cell_results)} genes."
            )
            ...
            logger.info(
                f"Significant genes for cell type {cell_type} (padj < {par['padj_threshold']}, |log2FC| > {par['log2fc_threshold']}): {len(significant_genes_cell)}"
            )
        ...
        # Perform statistical test
        stat_res = DeseqStats(dds, contrast=contrast_tuple)
        stat_res.summary()
        ...
        logger.info(
            f"Significant genes (padj < {par['padj_threshold']}, |log2FC| > {par['log2fc_threshold']}): {len(significant_genes)}"
        )
        ...
        results["significant"] = (results["padj"] < par["padj_threshold"]) & (
            results["log2FoldChange"].abs() > par["log2fc_threshold"]
        )
    # Save results
