...
def test_train_test(run_component, random_h5mu_path):
    ...
    assert (
        train_mudata.mod["rna"].n_obs + test_mudata.mod["rna"].n_obs
        == input_mudata.mod["rna"].n_obs
    ), "train and test data do not sum up to input data"

    assert (
        abs(train_mudata.mod["rna"].n_obs - round(input_mudata.mod["rna"].n_obs * 0.8))
        <= 1
    ), "train data has wrong size"
    assert (
        abs(test_mudata.mod["rna"].n_obs - round(input_mudata.mod["rna"].n_obs * 0.2))
        <= 1
...
def test_train_val_test(run_component, random_h5mu_path):
    ...
    assert (
        list(train_mudata.mod.keys())
        == list(val_mudata.mod.keys())
        == list(test_mudata.mod.keys())
        == ["rna"]
    )

    assert (
        train_mudata.mod["rna"].n_obs
        + val_mudata.mod["rna"].n_obs
        + test_mudata.mod["rna"].n_obs
        == input_mudata.mod["rna"].n_obs
    ), "train, val and test data do not sum up to input data"

    assert (
        abs(train_mudata.mod["rna"].n_obs - round(input_mudata.mod["rna"].n_obs * 0.7))
        <= 1
    ), "train data has wrong size"
    assert (
        abs(val_mudata.mod["rna"].n_obs - round(input_mudata.mod["rna"].n_obs * 0.1))
        <= 1
...
def test_raise_test_val_size(run_component):
    ...
    assert re.search(
        r"Sum of test_size and val_size must not exceed 1.",
...
def test_raise_invalid_val_out(run_component, random_h5mu_path):
    ...
    assert re.search(
        r"Both --val_size and --output_val must be set to use validation set.",
        err.value.stdout.decode("utf-8"),
    )
