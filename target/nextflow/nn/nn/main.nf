...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component nn")
      println("exiting now...")
        exit 1
    }
  }
...
process nn_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.nn.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow nn {
  ...
  result_ = nn_process(id_input_output_function_cli_params_) \
...
workflow test {
  ...
  main:
  params.test = true
  params.nn.output = "nn.log"

  Channel.from(rootDir) \
    | filter { params.nn.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.nn.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | nn
