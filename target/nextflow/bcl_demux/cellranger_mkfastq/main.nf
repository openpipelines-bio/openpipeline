...
// 
// Component authors:
//  * Toni Verbeiren <tverbeir@its.jnj.com> (contributor) {account: tverbeir}
...
index sets, and generate 10x-specific quality metrics after the demultiplex. Any
bcl2fastq argument will work (except a few that are set by the pipeline to
ensure proper trimming and sample indexing). The FASTQ output generated will be
the same as when running bcl2fastq directly.

Options:
    --input
        type: file
        Path of Illumina BCL run tar.gz or folder.

    --samplesheet
        type: file
        Path to the sample sheet. The sample sheet can either be a simple CSV
with lane, sample and index columns, or an Illumina Experiment
Manager-compatible sample sheet. Sample sheet indexes can refer to 10x sample
index set names (e.g., SI-GA-A12).

    --output
        type: file, required parameter, output
        The output folder generated by cellranger mkfastq

    --barcode_mismatches
        type: integer
        default: 1
        Number of allowed mismatches per index. Multiple, comma delimited,
entries allowed. Each entry is applied to the corresponding index; last entry
applies to all remaining indices. Accepted values: 0, 1, 2.

    --memory
        type: integer
        default: 128
        Set max GB the pipeline may request at one time. Only applies to local
jobs. Default=128. Set to '0' to use the default cellranger behaviour.

    --cores
        type: integer
        default: 32
        Set max cores the pipeline may request at one time. Only applies to
local jobs. Default=32. Set to '0' to use the default cellranger behaviour.'''
...
$( if [ ! -z ${VIASH_PAR_SAMPLESHEET+x} ]; then echo "par_samplesheet='${VIASH_PAR_SAMPLESHEET//\\'/\\'\\"\\'\\"\\'}'"; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT+x} ]; then echo "par_output='${VIASH_PAR_OUTPUT//\\'/\\'\\"\\'\\"\\'}'"; fi )
$( if [ ! -z ${VIASH_PAR_BARCODE_MISMATCHES+x} ]; then echo "par_barcode_mismatches='${VIASH_PAR_BARCODE_MISMATCHES//\\'/\\'\\"\\'\\"\\'}'"; fi )
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openpipelines-bio/bcl_demux_cellranger_mkfastq",
    "tag" : "main_build"
  }
}"""),
  // auto settings
