  ...
  "resources" : [
    {
      ...
      "path" : "script.sh",
      "is_executable" : true,
      "parent" : "file:/home/runner/work/openpipeline/openpipeline/src/mapping/cellranger_count_split/config.vsh.yaml"
    }
  ],
...
$( if [ ! -z ${VIASH_PAR_FILTERED_H5+x} ]; then echo "${VIASH_PAR_FILTERED_H5}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_filtered_h5='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_METRICS_SUMMARY+x} ]; then echo "${VIASH_PAR_METRICS_SUMMARY}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_metrics_summary='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_MOLECULE_INFO+x} ]; then echo "${VIASH_PAR_MOLECULE_INFO}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_molecule_info='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_BAM+x} ]; then echo "${VIASH_PAR_BAM}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_bam='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_BAI+x} ]; then echo "${VIASH_PAR_BAI}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_bai='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_RAW_H5+x} ]; then echo "${VIASH_PAR_RAW_H5}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_raw_h5='&'#" ; fi )
