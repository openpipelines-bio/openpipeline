...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component cellranger_count")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "cellranger_count." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process cellranger_count_process {
  label 'highmem'
  label 'highcpu'
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.cellranger_count.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow cellranger_count {
  ...
  result_ = cellranger_count_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.cellranger_count
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.cellranger_count.output = "cellranger_count.log"

  Channel.from(rootDir) \
    | filter { params.cellranger_count.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.cellranger_count.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | cellranger_count
