...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component bd_rhapsody_extracth5ad")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "bd_rhapsody_extracth5ad" + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process bd_rhapsody_extracth5ad_process {
  ...
  script:
    if (params.test)
      ...
      ./${params.bd_rhapsody_extracth5ad.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow bd_rhapsody_extracth5ad {
  ...
  def key = "bd_rhapsody_extracth5ad"
  ...
  result_ = bd_rhapsody_extracth5ad_process(id_input_output_function_cli_params_) \
...
workflow {
  def id = params.id
  def fname = "bd_rhapsody_extracth5ad"
  ...
  def inputFiles = params.bd_rhapsody_extracth5ad
    .arguments
  ...
  result = bd_rhapsody_extracth5ad(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.bd_rhapsody_extracth5ad.output = "bd_rhapsody_extracth5ad.log"

  Channel.from(rootDir) \
    | filter { params.bd_rhapsody_extracth5ad.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.bd_rhapsody_extracth5ad.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | bd_rhapsody_extracth5ad

  emit:
  bd_rhapsody_extracth5ad.out
}