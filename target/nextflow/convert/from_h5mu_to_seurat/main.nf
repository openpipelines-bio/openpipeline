...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component from_h5mu_to_seurat")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "from_h5mu_to_seurat." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process from_h5mu_to_seurat_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.from_h5mu_to_seurat.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow from_h5mu_to_seurat {
  ...
  result_ = from_h5mu_to_seurat_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.from_h5mu_to_seurat
    .arguments
  ...
  result = from_h5mu_to_seurat(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.from_h5mu_to_seurat.output = "from_h5mu_to_seurat.log"

  Channel.from(rootDir) \
    | filter { params.from_h5mu_to_seurat.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.from_h5mu_to_seurat.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | from_h5mu_to_seurat

  emit:
  from_h5mu_to_seurat.out
}