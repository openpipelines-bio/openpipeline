// from_bd_to_10x_molecular_barcode_tags main_build
// 
...
$( if [ ! -z ${VIASH_PAR_BAM+x} ]; then echo "${VIASH_PAR_BAM}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_bam='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_THREADS+x} ]; then echo "${VIASH_PAR_THREADS}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_threads='&'#" ; fi )
