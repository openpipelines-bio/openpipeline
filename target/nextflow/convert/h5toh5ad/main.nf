...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component h5toh5ad")
      println("exiting now...")
        exit 1
    }
  }
...
process h5toh5ad_process {
  ...
  script:
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.h5toh5ad.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow h5toh5ad {
  ...
  result_ = h5toh5ad_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def inputFiles = params.h5toh5ad
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.h5toh5ad.output = "h5toh5ad.log"

  Channel.from(rootDir) \
    | filter { params.h5toh5ad.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.h5toh5ad.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | h5toh5ad
