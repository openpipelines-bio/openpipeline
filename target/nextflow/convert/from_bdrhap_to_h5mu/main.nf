// from_bdrhap_to_h5mu main_build
// 
...
thisHelpMessage = '''from_bdrhap_to_h5mu main_build
...
Options:
    --input
        ...
        example: input_dir
        One or more folders containing the output of the BD Rhapsody workflow.
If multiple are given, they are all combined into one h5mu file. Note that
multiple outputs must come from the same sample (but e.g. from different lanes).
