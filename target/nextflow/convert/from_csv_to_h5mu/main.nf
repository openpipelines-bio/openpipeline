...
thisHelpMessage = '''from_csv_to_h5mu main_build

Converts an csv into an h5ad file.
...
Options:
    ...
    --delimiter
        type: string
        default: ,
        Delimiter for the different fields in the delimited file.

    --use_column_names
        type: boolean
        default: true
        Use the first row as column names as gene names'''
...
par = {
  ...
  'delimiter': $( if [ ! -z ${VIASH_PAR_DELIMITER+x} ]; then echo "'${VIASH_PAR_DELIMITER//\\'/\\\\\\'}'"; else echo None; fi ),
  'use_column_names': $( if [ ! -z ${VIASH_PAR_USE_COLUMN_NAMES+x} ]; then echo "'${VIASH_PAR_USE_COLUMN_NAMES//\\'/\\\\\\'}'.lower() == 'true'"; else echo None; fi )
}
meta = {
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openpipelines-bio/convert_from_csv_to_h5mu",
    "tag" : "main_build"
  }
}"""),
  // auto settings
