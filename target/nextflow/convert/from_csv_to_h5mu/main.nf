...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component from_csv_to_h5mu")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "from_csv_to_h5mu." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process from_csv_to_h5mu_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.from_csv_to_h5mu.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow from_csv_to_h5mu {
  ...
  result_ = from_csv_to_h5mu_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.from_csv_to_h5mu
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.from_csv_to_h5mu.output = "from_csv_to_h5mu.log"

  Channel.from(rootDir) \
    | filter { params.from_csv_to_h5mu.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.from_csv_to_h5mu.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | from_csv_to_h5mu

  emit:
  from_csv_to_h5mu.out
}