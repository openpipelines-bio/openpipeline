...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component find_neighbors")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "find_neighbors." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process find_neighbors_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.find_neighbors.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow find_neighbors {
  ...
  result_ = find_neighbors_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.find_neighbors
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.find_neighbors.output = "find_neighbors.log"

  Channel.from(rootDir) \
    | filter { params.find_neighbors.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.find_neighbors.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | find_neighbors
