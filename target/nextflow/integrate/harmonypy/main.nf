...
// 
// Component authors:
//  * Dries Schaumont <dries.schaumont@its.jnj.com> (maintainer)
...
thisConfig = processConfig([
  ...
  "arguments" : [
    ...
    },
    {
      ...
      "description" : "Suffix for the key in .obsm where the results will be stored. The resulting key will be a comination of the pca key and the output key suffix.",
      "default" : [
        "harmonypy"
      ],
      "required" : false,
...
par = {
  ...
  'pca_key': $( if [ ! -z ${VIASH_PAR_PCA_KEY+x} ]; then echo "'${VIASH_PAR_PCA_KEY//\\'/\\\\\\'}'"; else echo None; fi ),
  'output_key_suffix': $( if [ ! -z ${VIASH_PAR_OUTPUT_KEY_SUFFIX+x} ]; then echo "'${VIASH_PAR_OUTPUT_KEY_SUFFIX//\\'/\\\\\\'}'"; else echo None; fi ),
  'theta': $( if [ ! -z ${VIASH_PAR_THETA+x} ]; then echo "list(map(float, '${VIASH_PAR_THETA//\\'/\\\\\\'}'.split(':')))"; else echo None; fi ),
  'sample_key': $( if [ ! -z ${VIASH_PAR_SAMPLE_KEY+x} ]; then echo "'${VIASH_PAR_SAMPLE_KEY//\\'/\\\\\\'}'.split(':')"; else echo None; fi )
}
meta = {
