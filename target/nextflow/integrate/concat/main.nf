...
thisHelpMessage = '''concat main_build
...
Options:
    ...
    --sample_names
        type: string, required parameter, multiple values allowed
        default: sample_names
        Names of the different samples that have to be concatenated.
    ...
    --other_axis_mode
        ...
        How to handle the merging of other axis (var, obs, ...).
         - None: keep no data
         - same: only keep elements of the matrices which are the same in each
of the samples
         - unique: only keep elements for which there is only 1 possible value
(1 value that can occur in multiple samples)
         - first: keep the annotation from the first sample
         - only: keep elements that show up in only one of the objects (1 unique
element in only 1 sample)
         - concat: concatenate unique values together'''
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openpipelines-bio/integrate_concat",
    "tag" : "main_build"
  },
  "label" : [
    "highmem"
