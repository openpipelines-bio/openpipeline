...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component do_filter")
      println("exiting now...")
        exit 1
    }
  }
...
process do_filter_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.do_filter.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow do_filter {
  ...
  result_ = do_filter_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.do_filter
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.do_filter.output = "do_filter.log"

  Channel.from(rootDir) \
    | filter { params.do_filter.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.do_filter.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | do_filter
