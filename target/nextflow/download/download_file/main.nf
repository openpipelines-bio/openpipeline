...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component download_file")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "download_file." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process download_file_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.download_file.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow download_file {
  ...
  result_ = download_file_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.download_file
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.download_file.output = "download_file.log"

  Channel.from(rootDir) \
    | filter { params.download_file.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.download_file.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | download_file
