...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component sync_test_resources")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "sync_test_resources." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process sync_test_resources_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.sync_test_resources.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow sync_test_resources {
  ...
  result_ = sync_test_resources_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.sync_test_resources
    .arguments
  ...
  result = sync_test_resources(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.sync_test_resources.output = "sync_test_resources.log"

  Channel.from(rootDir) \
    | filter { params.sync_test_resources.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.sync_test_resources.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | sync_test_resources

  emit:
  sync_test_resources.out
}