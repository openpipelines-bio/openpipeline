...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component leiden")
      println("exiting now...")
        exit 1
    }
  }
...
process leiden_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.leiden.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow leiden {
  ...
  result_ = leiden_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.leiden.output = "leiden.log"

  Channel.from(rootDir) \
    | filter { params.leiden.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.leiden.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | leiden
