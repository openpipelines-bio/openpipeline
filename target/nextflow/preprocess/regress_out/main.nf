...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component regress_out")
      println("exiting now...")
        exit 1
    }
  }
...
process regress_out_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.regress_out.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow regress_out {
  ...
  result_ = regress_out_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.regress_out
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.regress_out.output = "regress_out.log"

  Channel.from(rootDir) \
    | filter { params.regress_out.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.regress_out.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | regress_out
