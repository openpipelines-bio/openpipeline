...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component mermaid")
      println("exiting now...")
        exit 1
    }
  }
...
process mermaid_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.mermaid.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow mermaid {
  ...
  result_ = mermaid_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.mermaid.output = "mermaid.log"

  Channel.from(rootDir) \
    | filter { params.mermaid.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.mermaid.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | mermaid
