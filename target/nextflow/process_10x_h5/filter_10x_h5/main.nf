...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component filter_10x_h5")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "filter_10x_h5." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process filter_10x_h5_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.filter_10x_h5.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow filter_10x_h5 {
  ...
  result_ = filter_10x_h5_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.filter_10x_h5
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.filter_10x_h5.output = "filter_10x_h5.log"

  Channel.from(rootDir) \
    | filter { params.filter_10x_h5.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.filter_10x_h5.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | filter_10x_h5
