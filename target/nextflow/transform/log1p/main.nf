...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component log1p")
      println("exiting now...")
        exit 1
    }
  }
...
process log1p_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.log1p.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow log1p {
  ...
  result_ = log1p_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.log1p.output = "log1p.log"

  Channel.from(rootDir) \
    | filter { params.log1p.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.log1p.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | log1p
