...
thisHelpMessage = '''normalize_total main_build

Normalize counts per cell.

Normalize each cell by total counts over all genes, so that every cell has the
same total count after normalization. If choosing target_sum=1e6, this is CPM
normalization.
...
meaningful as these can strongly influence the resulting normalized values for
all other genes [Weinreb17].
...
equal to the median of total counts for observations (cells) before
normalization.
...
expressed, if it has more than max_fraction of the total counts in at least one
cell. The not-excluded genes will sum up to target_sum.'''
