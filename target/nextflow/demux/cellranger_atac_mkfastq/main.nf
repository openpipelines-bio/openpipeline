// cellranger_atac_mkfastq atac_build
// 
...
meta = [
  ...
  "functionality" : {
    ...
    "arguments" : [
      ...
      },
      {
        "type" : "string",
        "name" : "--lanes",
        "description" : "bcl2fastq option. Comma-delimited series of lanes to demultiplex. Use this if you have a sample sheet for an entire flow cell but only want to generate a few lanes for further 10x Genomics analysis.",
        "example" : [
          "1,3"
        ],
        "required" : false,
    ...
    "resources" : [
      {
        ...
        "path" : "script.sh",
        "is_executable" : true,
        "parent" : "file:/home/runner/work/openpipeline/openpipeline/src/demux/cellranger_atac_mkfastq/"
      },
      {
        "type" : "file",
  ...
  "info" : {
    "config" : "/home/runner/work/openpipeline/openpipeline/src/demux/cellranger_atac_mkfastq/config.vsh.yaml",
    "platform" : "nextflow",
    "output" : "/home/runner/work/openpipeline/openpipeline/target/nextflow/demux/cellranger_atac_mkfastq",
    "viash_version" : "0.8.5",
...
$( if [ ! -z ${VIASH_PAR_CSV+x} ]; then echo "${VIASH_PAR_CSV}" | sed "s#'#'\\"'\\"'#g;s#.*#par_csv='&'#" ; else echo "# par_csv="; fi )
$( if [ ! -z ${VIASH_PAR_LANES+x} ]; then echo "${VIASH_PAR_LANES}" | sed "s#'#'\\"'\\"'#g;s#.*#par_lanes='&'#" ; else echo "# par_lanes="; fi )
$( if [ ! -z ${VIASH_PAR_USE_BASES_MASK+x} ]; then echo "${VIASH_PAR_USE_BASES_MASK}" | sed "s#'#'\\"'\\"'#g;s#.*#par_use_bases_mask='&'#" ; else echo "# par_use_bases_mask="; fi )
$( if [ ! -z ${VIASH_PAR_DELETE_UNDETERMINED+x} ]; then echo "${VIASH_PAR_DELETE_UNDETERMINED}" | sed "s#'#'\\"'\\"'#g;s#.*#par_delete_undetermined='&'#" ; else echo "# par_delete_undetermined="; fi )
$( if [ ! -z ${VIASH_PAR_BARCODE_MISMATCHES+x} ]; then echo "${VIASH_PAR_BARCODE_MISMATCHES}" | sed "s#'#'\\"'\\"'#g;s#.*#par_barcode_mismatches='&'#" ; else echo "# par_barcode_mismatches="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openpipelines-bio/demux_cellranger_atac_mkfastq",
    "tag" : "atac_build"
  },
  "label" : [
    "highmem",
