...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component hvg_scanpy")
      println("exiting now...")
        exit 1
    }
  }
...
process hvg_scanpy_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.hvg_scanpy.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow hvg_scanpy {
  ...
  result_ = hvg_scanpy_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.hvg_scanpy
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.hvg_scanpy.output = "hvg_scanpy.log"

  Channel.from(rootDir) \
    | filter { params.hvg_scanpy.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.hvg_scanpy.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | hvg_scanpy
